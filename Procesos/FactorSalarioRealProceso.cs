...
namespace ERP_TECKIO
{
    public class FactorSalarioRealProceso<TContext> where TContext : DbContext
    {
        ...
        public async Task<List<ParametrosFsrXInsumoDTO>> obtenerParametrosXInsumo(FactorSalarioRealDTO fsr) {
            var parametrosXInsumo = new List<ParametrosFsrXInsumoDTO>();
            ...
            if (parametros.Id <= 0)
            {
                ...
                for (int y = 0; y < insumoFiltrados.Count(); y++)
                {
                    ...
                    {
                        IdInsumo = insumoFiltrados[y].id,
                        Descripcion = insumoFiltrados[y].Descripcion,
                        CostoBase = insumoFiltrados[y].CostoBase,
                        CostoUnitario = insumoFiltrados[y].CostoUnitario,
                        RiesgoTrabajo = 0,
                        CuotaFija = 0,
                ...
                else
                {
                    ...
                    for (int y = 0; y < insumoFiltrados.Count(); y++)
                    {
                        ...
                        parametrosXInsumo.Add(new ParametrosFsrXInsumoDTO()
                        {
                            ...
                            CuotaFija = cuotaFija,
                            AplicacionExcedente = aplicacionExcedente,
                            PrestacionDinero = prestacionDinero,
                            GastoMedico = gastoMedico,
                            InvalidezVida = invalidezVida,
                            Retiro = retiro,
                            CesantiaEdad = cesantiaEdad,
                            PrestaconSocial = prestacionSocial,
                            Infonavit = infinavit,
                            SalarioBaseCotizacion = SalarioBaseCotizacion,
                            SumaPrestaciones = sumaPresataciones,
                            SumaSOBRECostoBase = sumaSOBREcostoBase,
                            FsrInsumo = FsrInsumo,
                            IdProyecto = insumoFiltrados[y].IdProyecto
                        });
                    }
                }
            }

            return parametrosXInsumo;
        }

        public async Task actualizarCostoBaseInsumo(ParametrosFsrXInsumoDTO parametroXInsumo) {
            

            var insumos = await _InsumoService.ObtenXIdProyecto(parametroXInsumo.IdProyecto);
            var insumo = insumos.FirstOrDefault(z => z.id == parametroXInsumo.IdInsumo);

            var insumosFiltrados = insumos.Where(z => z.Codigo == insumo.Codigo);

            var FSRs = await _FSRService.ObtenerTodosXProyecto(parametroXInsumo.IdProyecto);
            if (FSRs.Count <= 0) {
                return;
            }
            var fsr = FSRs.FirstOrDefault();

            foreach (var insumoF in insumosFiltrados) {
                insumoF.CostoBase = parametroXInsumo.CostoBase;
                await actualizarCostoUnitarioInsumoMO(insumoF, fsr);
            }
        }

        public async Task<decimal> obtenerCostoUnitarioConFsrXCostoBase(decimal costoBase, FactorSalarioRealDTO fsr) {
            ...
            else
            {
                ...
                else
                {
                    ...
                    decimal FSBaseCotizacion = diasPagados / 365;
                    decimal SalarioBaseCotizacion = costoBase * FSBaseCotizacion;
                    ...
                    {
                        decimal rangoUma = costoBase / parametros.UMA;
                        var rangoActuales = pocentajesCesantia.Where(z => z.RangoUMA <= rangoUma);
                        var ordenado = rangoActuales.OrderByDescending(z => z.RangoUMA);
                        var rangoUbicado = ordenado.FirstOrDefault();
                    ...
                    costoUnitario = (costoBase * FsrInsumo);
                    return costoUnitario;
                }
            }
        }
