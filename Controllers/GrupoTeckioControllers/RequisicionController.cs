...
namespace ERP_TECKIO.Controllers
{
    ...
    /// </summary>
    [Route("api/requisicion/2")]
    [ApiController]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
    public class RequisicionGrupoTeckioController : ControllerBase
    {
        private readonly IRequisicionService<GrupoTeckioContext> _RequisicionService;
        private readonly RequisicionProceso<GrupoTeckioContext> _Proceso;
        private readonly ObjetoRequisicionProceso<GrupoTeckioContext> _ObjetoRequisicionProceso;
        private readonly IProyectoService<GrupoTeckioContext> _ProyectoService;
        //private readonly IMapper _mapper;
        /// <summary>
        /// Se usa para mostrar errores en la consola
        /// </summary>
        private readonly ILogger<RequisicionGrupoTeckioController> _Logger;
        /// <summary>
        ...
        public RequisicionGrupoTeckioController(
            ILogger<RequisicionGrupoTeckioController> Logger
            , GrupoTeckioContext Context
            , IRequisicionService<GrupoTeckioContext> RequisicionService
            , RequisicionProceso<GrupoTeckioContext> Proceso
            , ObjetoRequisicionProceso<GrupoTeckioContext> ObjetoRequisicionProceso
            , IProyectoService<GrupoTeckioContext> ProyectoService
            )
        {
            _Logger = Logger;
            _Context = Context;
        ...
        public async Task<ActionResult> Put([FromBody] RequisicionDTO parametros)
        {
            try
            {
                ...
                await HttpContext.InsertarParametrosCreacionEdicionEnCabecera<GrupoTeckioContext>(resultado.Estatus, resultado.Descripcion);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RequisicionDTO>>> filtrar([FromBody] RequisicionBusquedaExtensaDTO FiltrarDTO)
        {
            ...
            if (Convert.ToDateTime(FiltrarDTO.FechaFinal) != Convert.ToDateTime(null)
                ...
                && Convert.ToDateTime(FiltrarDTO.FechaFinal) > Convert.ToDateTime("01/01/2/970"))
            {
