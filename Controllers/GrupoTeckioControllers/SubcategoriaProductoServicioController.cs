...
namespace ERP_TECKIO.Controllers.GrupoTeckio
{
    ...
    [Route("api/subcategoriaProductoServicio/2")]
    [ApiController]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
    public class SubcategoriaProductoServicioController: ControllerBase
    {
        private readonly ISubcategoriaProdutoYServicio<GrupoTeckioContext> _service;

        public SubcategoriaProductoServicioController(ISubcategoriaProdutoYServicio<GrupoTeckioContext> service)
        {
            _service = service;
        }
