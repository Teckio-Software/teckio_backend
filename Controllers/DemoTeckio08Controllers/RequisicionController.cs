...
namespace ERP_TECKIO.Controllers
{
    ...
    /// </summary>
    [Route("api/requisicion/8")]
    [ApiController]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
    public class RequisicionDemoTeckioAL08Controller : ControllerBase
    {
        private readonly IRequisicionService<DemoTeckioAL08Context> _RequisicionService;
        private readonly RequisicionProceso<DemoTeckioAL08Context> _Proceso;
        private readonly ObjetoRequisicionProceso<DemoTeckioAL08Context> _ObjetoRequisicionProceso;
        private readonly IProyectoService<DemoTeckioAL08Context> _ProyectoService;
        //private readonly IMapper _mapper;
        /// <summary>
        /// Se usa para mostrar errores en la consola
        /// </summary>
        private readonly ILogger<RequisicionDemoTeckioAL08Controller> _Logger;
        /// <summary>
        ...
        public RequisicionDemoTeckioAL08Controller(
            ILogger<RequisicionDemoTeckioAL08Controller> Logger
            , DemoTeckioAL08Context Context
            , IRequisicionService<DemoTeckioAL08Context> RequisicionService
            , RequisicionProceso<DemoTeckioAL08Context> Proceso
            , ObjetoRequisicionProceso<DemoTeckioAL08Context> ObjetoRequisicionProceso
            , IProyectoService<DemoTeckioAL08Context> ProyectoService
            )
        {
            _Logger = Logger;
            _Context = Context;
        ...
        public async Task<ActionResult> Put([FromBody] RequisicionDTO parametros)
        {
            try
            {
                ...
                await HttpContext.InsertarParametrosCreacionEdicionEnCabecera<DemoTeckioAL08Context>(resultado.Estatus, resultado.Descripcion);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RequisicionDTO>>> filtrar([FromBody] RequisicionBusquedaExtensaDTO FiltrarDTO)
        {
            ...
            if (Convert.ToDateTime(FiltrarDTO.FechaFinal) != Convert.ToDateTime(null)
                ...
                && Convert.ToDateTime(FiltrarDTO.FechaFinal) > Convert.ToDateTime("01/01/8/870"))
            {
