...
namespace SistemaERP.API.Alumno44Controllers.Procomi
{
    ...
    /// </summary>
    [Route("api/compradirecta/44")]
    [ApiController]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]//, Policy = "SeccionOrdenCompra-Empresa35")]
    public class CompraDirectaAlumno44Controller : ControllerBase
    {
        private readonly ICompraDirectaService<Alumno44Context> _Service;
        /// <summary>
        /// Se usa para mostrar errores en consola
        /// </summary>
        private readonly ILogger<CompraDirectaAlumno44Controller> Logger;
        /// <summary>
        ...
        public CompraDirectaAlumno44Controller(
            ILogger<CompraDirectaAlumno44Controller> logger,
            Alumno44Context context
            , ICompraDirectaService<Alumno44Context> Service)
        {
            Logger = logger;
            Context = context;
            _Service = Service;
        ...
        public async Task<ActionResult> Post([FromBody] CompraDirectaCreacionDTO CreacionDTO)
        {
            try
            {
                ...
                await HttpContext.InsertarParametrosCreacionEdicionEnCabecera<Alumno44Context>(resultado.Estatus, resultado.Descripcion);
            }
            catch (Exception ex)
            {
