...
{
    [Route("api/cotizacion/3")]
    [ApiController]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
    public class CotizacionDemoTeckioController : ControllerBase
    {
        private readonly ICotizacionService<DemoTeckioContext> _Service;
        private readonly CotizacionProceso<DemoTeckioContext> _Proceso;
        private readonly IOrdenCompraService<DemoTeckioContext> _OrdenCompraService;
        private readonly ITipoImpuestoService<DemoTeckioContext> _TipoImpuestoService;
        private readonly ILogger<CotizacionDemoTeckioController> _Logger;
        private readonly DemoTeckioContext Context;
        public CotizacionDemoTeckioController(

            ILogger<CotizacionDemoTeckioController> Logger
            , DemoTeckioContext Context
            , ICotizacionService<DemoTeckioContext> Service
            , IOrdenCompraService<DemoTeckioContext> OrdenCompraService,
              ITipoImpuestoService<DemoTeckioContext> TipoImpuestoService
            , CotizacionProceso<DemoTeckioContext> Proceso)
        {
            _Logger = Logger;
        ...
        public async Task<ActionResult> Post([FromBody] CotizacionCreacionDTO creacionDTO)
        {
            try
            {
                ...
                await HttpContext.InsertarParametrosCreacionEdicionEnCabecera<DemoTeckioContext>(resultado.Estatus, resultado.Descripcion);
            }
            catch (Exception ex)
            {
