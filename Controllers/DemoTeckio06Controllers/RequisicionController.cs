...
namespace ERP_TECKIO.Controllers
{
    ...
    /// </summary>
    [Route("api/requisicion/6")]
    [ApiController]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
    public class RequisicionDemoTeckioAL06Controller : ControllerBase
    {
        private readonly IRequisicionService<DemoTeckioAL06Context> _RequisicionService;
        private readonly RequisicionProceso<DemoTeckioAL06Context> _Proceso;
        private readonly ObjetoRequisicionProceso<DemoTeckioAL06Context> _ObjetoRequisicionProceso;
        private readonly IProyectoService<DemoTeckioAL06Context> _ProyectoService;
        //private readonly IMapper _mapper;
        /// <summary>
        /// Se usa para mostrar errores en la consola
        /// </summary>
        private readonly ILogger<RequisicionDemoTeckioAL06Controller> _Logger;
        /// <summary>
        ...
        public RequisicionDemoTeckioAL06Controller(
            ILogger<RequisicionDemoTeckioAL06Controller> Logger
            , DemoTeckioAL06Context Context
            , IRequisicionService<DemoTeckioAL06Context> RequisicionService
            , RequisicionProceso<DemoTeckioAL06Context> Proceso
            , ObjetoRequisicionProceso<DemoTeckioAL06Context> ObjetoRequisicionProceso
            , IProyectoService<DemoTeckioAL06Context> ProyectoService
            )
        {
            _Logger = Logger;
            _Context = Context;
        ...
        public async Task<ActionResult> Put([FromBody] RequisicionDTO parametros)
        {
            try
            {
                ...
                await HttpContext.InsertarParametrosCreacionEdicionEnCabecera<DemoTeckioAL06Context>(resultado.Estatus, resultado.Descripcion);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RequisicionDTO>>> filtrar([FromBody] RequisicionBusquedaExtensaDTO FiltrarDTO)
        {
            ...
            if (Convert.ToDateTime(FiltrarDTO.FechaFinal) != Convert.ToDateTime(null)
                ...
                && Convert.ToDateTime(FiltrarDTO.FechaFinal) > Convert.ToDateTime("01/01/6/970"))
            {
