...
namespace ERP_TECKIO.Controllers
{
    ...
    /// </summary>
    [Route("api/requisicion/4")]
    [ApiController]
    [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]
    public class RequisicionDemoTeckioAL04Controller : ControllerBase
    {
        private readonly IRequisicionService<DemoTeckioAL04Context> _RequisicionService;
        private readonly RequisicionProceso<DemoTeckioAL04Context> _Proceso;
        private readonly ObjetoRequisicionProceso<DemoTeckioAL04Context> _ObjetoRequisicionProceso;
        private readonly IProyectoService<DemoTeckioAL04Context> _ProyectoService;
        //private readonly IMapper _mapper;
        /// <summary>
        /// Se usa para mostrar errores en la consola
        /// </summary>
        private readonly ILogger<RequisicionDemoTeckioAL04Controller> _Logger;
        /// <summary>
        ...
        public RequisicionDemoTeckioAL04Controller(
            ILogger<RequisicionDemoTeckioAL04Controller> Logger
            , DemoTeckioAL04Context Context
            , IRequisicionService<DemoTeckioAL04Context> RequisicionService
            , RequisicionProceso<DemoTeckioAL04Context> Proceso
            , ObjetoRequisicionProceso<DemoTeckioAL04Context> ObjetoRequisicionProceso
            , IProyectoService<DemoTeckioAL04Context> ProyectoService
            )
        {
            _Logger = Logger;
            _Context = Context;
        ...
        public async Task<ActionResult> Put([FromBody] RequisicionDTO parametros)
        {
            try
            {
                ...
                await HttpContext.InsertarParametrosCreacionEdicionEnCabecera<DemoTeckioAL04Context>(resultado.Estatus, resultado.Descripcion);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RequisicionDTO>>> filtrar([FromBody] RequisicionBusquedaExtensaDTO FiltrarDTO)
        {
            ...
            if (Convert.ToDateTime(FiltrarDTO.FechaFinal) != Convert.ToDateTime(null)
                ...
                && Convert.ToDateTime(FiltrarDTO.FechaFinal) > Convert.ToDateTime("01/01/4/970"))
            {
