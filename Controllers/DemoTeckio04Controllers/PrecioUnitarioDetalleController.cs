...
namespace SistemaERP.API.DemoTeckioAL04Controllers.Procomi
{
    [Route("api/preciounitariodetalle/4")]
    [ApiController]
    public class PrecioUnitarioDetalleDemoTeckioAL04Controller : ControllerBase
    {
        ...
        public PrecioUnitarioDetalleDemoTeckioAL04Controller(
        ...
        [HttpPost("eliminarMultiple")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme)]//, Policy = "EliminarPrecioUnitario-Empresa2")]
        public async Task<ActionResult<List<PrecioUnitarioDetalleDTO>>> EliminarMultiple([FromBody] List<int> Ids)
        {
            var registros = new List<PrecioUnitarioDetalleDTO>();
            using (var db = new DemoTeckioAL04Context(_Options.Options))
            {
